...
const rowField = (id: number, field: keyof RowValues) => `rows.${id}.${field}` as const;
const rowInputId = (id: number, field: keyof RowValues) => `rel-issue-${id}-${field}`;
...
function hasMessage<T extends { message?: string }>(item: T): item is T & { message: string } {
 ...
 */
function ScoreEditor({ relationship, onClose }: { relationship: Relationship; onClose: () => void }) {
...
function ScoreEditorForm({
  ...
  const seed = useMemo(() => {
    const saved = new Map(relationship.issues.map((issue) => [issue.issue_id, issue]));
  ...
  const refresh = () => {
    ...
    void queryClient.invalidateQueries({ queryKey: ["figure", relationship.source_id] });
    void queryClient.invalidateQueries({ queryKey: ["figure", relationship.target_id] });
  ...
  const clearIssue = useMutation({
    mutationFn: (issueId: number) => api.relationships.removeIssue(relationship.id, issueId),
    onSuccess: (_data, issueId) => {
  ...
  const addModifier = useMutation({
    mutationFn: (values: ModifierForm) =>
      api.relationships.addModifier(relationship.id, {
        ...
        value: values.value,
        kind: values.kind,
        expires_at: values.expires_at ? `${values.expires_at}T23:59:59+00:00` : null,
      }),
    onSuccess: () => {
      modifierForm.reset({
        label: "",
        value: 0,
        kind: "event",
        expires_at: new Date(Date.now() + 180 * 86_400_000).toISOString().slice(0, 10),
      });
      setServerError(null);
      refresh();
    },
  ...
  const preview = useMemo(() => {
    ...
    const modifierTotal = relationship.modifiers.reduce((sum, m) => sum + m.effective_value, 0);
  ...
  return (
    <Panel>
      ...
      <div className="mb-5 flex flex-wrap items-center gap-x-6 gap-y-2 rounded-sm border border-rule bg-neutral-sunk px-4 py-3 text-[13px]">
        <span>
          Isu terisi: <strong className="tabular font-semibold">{preview.filled}</strong>
        </span>
        <span>
          Dasar (rata-rata berbobot):{" "}
          <strong className="tabular font-semibold" style={{ color: scoreColor(preview.base) }}>
        ...
        <span>
          Skor akhir:{" "}
          <strong className="tabular font-semibold" style={{ color: scoreColor(preview.total) }}>
        ...
        {relationship.score_mode !== "computed" ? (
          <span className="rounded-sm border border-rule bg-neutral-raised px-2 py-0.5 text-[11px]">
            mode {relationship.score_mode}, skor publik memakai {relationship.manual_score}
          </span>
        ) : null}
      </div>
      ...
      <ul className="flex flex-col gap-3">
        {visible.map((issue) => {
          ...
          return (
            <li key={issue.id} className="rounded-md border border-rule bg-neutral-sunk/40 p-4">
              <div className="flex flex-wrap items-baseline justify-between gap-2">
                <div>
                  <span className="text-[13.5px] font-semibold">{issue.name}</span>
                  <span className="ml-2 text-[11.5px] text-ink-soft">
                    {issue.category ?? "–"} · bobot bawaan ×{issue.default_weight}
                    {isSaved ? " · sudah diisi" : ""}
                  </span>
                </div>
                <div className="flex items-center gap-3">
                  <ScoreValue score={row.score} color={scoreColor(row.score)} size="md" />
                  {isSaved ? (
                    <Button
                      variant="danger"
              ...
              <div className="mt-3 flex items-center gap-3">
                <input
                  {...form.register(rowField(issue.id, "score"), { valueAsNumber: true })}
                  ...
                  aria-describedby={scoreError ? `${rowInputId(issue.id, "score")}-error` : undefined}
              ...
              <div className="mt-3 grid gap-3 sm:grid-cols-[130px_1fr]">
                <div>
                  <Field label="Bobot">
                    <input
                      {...form.register(rowField(issue.id, "weight"), { valueAsNumber: true })}
                      ...
                      aria-describedby={
                        weightError ? `${rowInputId(issue.id, "weight")}-error` : undefined
                      }
                    />
                  </Field>
                  <FieldError
      ...
      {!showAll && emptyIssues.length > 0 ? (
        <p className="mt-3 text-[12.5px] text-ink-soft">
          {emptyIssues.length} isu belum diisi dan disembunyikan. Klik “Buka semua” untuk
          mengisinya.
        </p>
      ) : null}
      ...
      <p className="mb-3 text-[12.5px] text-ink-soft">
        Peristiwa memberi tambahan sementara pada skor. Nilainya memudar otomatis saat mendekati
        tanggal kedaluwarsa.
      </p>
          ...
          {relationship.modifiers.map((modifier) => (
            ...
            >
              <div className="min-w-0 flex-1">
                <div className="text-[13px] font-semibold">{modifier.label}</div>
                <div className="text-[11.5px] text-ink-soft">
                  {modifier.kind} · nilai asli {formatSigned(modifier.value, 0)} · efektif{" "}
                  {formatSigned(modifier.effective_value)} (faktor {modifier.fade})
                  {modifier.expires_at ? ` · sampai ${modifier.expires_at.slice(0, 10)}` : " · permanen"}
                </div>
              </div>
              {modifier.id !== null ? (
                <Button
      ...
      >
        ...
        <div>
          <Field label="Peristiwa baru">
            <input
              ...
              aria-describedby={modifierErrors.label ? "modifier-label-error" : undefined}
            />
          </Field>
          <FieldError id="modifier-label-error" message={modifierErrors.label?.message} />
        </div>
        <div>
          <Field label="Nilai">
            <input
              ...
              aria-describedby={modifierErrors.value ? "modifier-value-error" : undefined}
            />
          </Field>
          <FieldError id="modifier-value-error" message={modifierErrors.value?.message} />
        </div>
        <Field label="Jenis">
          <select {...modifierForm.register("kind")} id="modifier-kind" className={inputClass}>
            {["event", "scandal", "deal", "betrayal", "support", "endorsement", "legal"].map(
              (kind) => (
        ...
        <div className="flex items-end">
          <Button variant="primary" type="submit" disabled={addModifier.isPending}>
...
export function AdminRelationships() {
  ...
  if (editing) return <ScoreEditor relationship={editing} onClose={() => setEditing(null)} />;
      ...
      <Panel>
        ...
        {creating ? (
          ...
          >
            ...
            <div>
              <Field label="Figur A">
                <select
                  ...
                  aria-describedby={createErrors.source_id ? "rel-source-error" : undefined}
                >
              ...
              <FieldError id="rel-source-error" message={createErrors.source_id?.message} />
            </div>
            <div>
              <Field label="Figur B">
                <select
                  ...
                  aria-describedby={createErrors.target_id ? "rel-target-error" : undefined}
                >
                  <option value={0}>Pilih figur…</option>
                  {figures.map((f) => (
                    <option key={f.id} value={f.id}>
              ...
              <FieldError id="rel-target-error" message={createErrors.target_id?.message} />
            </div>
            <Field label="Jenis">
              <select {...createForm.register("rel_type")} id="rel-type" className={inputClass}>
                {["political", "coalition", "family", "business", "party", "government"].map(
                  (type) => (
            ...
            <div className="flex items-end">
              <Button variant="primary" type="submit" disabled={createRel.isPending}>
            ...
            <table className="w-full border-collapse text-[13px]">
              ...
              <thead>
                <tr className="border-b border-rule text-left">
                  {["Figur A", "Figur B", "Skor", "Dasar", "Modifier", "Isu", "Tingkat"].map(
                    (head) => (
                      <th
                        key={head}
                        scope="col"
              ...
              <tbody>
                {filtered.map((rel) => (
                  <tr key={rel.id} className="border-b border-rule/60 hover:bg-neutral-sunk/50">
                    ...
                    <td className="px-3 py-2.5">
                      <div className="flex items-center gap-2">
                        <ScoreValue score={rel.score} color={scoreColor(rel.score)} size="sm" />
                    ...
                    <td className="px-3 py-2.5">
                      <TierChip label={rel.tier.label} color={tierColor(rel.tier)} />
                    </td>
      ...
      {deleting ? (
        <ConfirmDialog
          ...
          body={
            <>
              Hapus relasi antara{" "}
              <strong className="font-semibold text-ink">{deleting.source_name}</strong> dan{" "}
              <strong className="font-semibold text-ink">{deleting.target_name}</strong>? Semua skor
              isu dan peristiwa pada relasi ini ikut terhapus.
            </>
          }
